...
//   - Navigation to data pages (/analytics, /dashboard): network-first.
...
// Data-heavy pages that should always fetch fresh content (network-first).
const FRESH_PAGES = ['/dashboard', '/analytics'];
...
self.addEventListener('fetch', (event) => {
  ...
  // Navigation requests to data pages (e.g. /analytics, /dashboard):
  // network-first so the user always sees fresh data. Falls back to cache
  // or a generic offline page — never lets the rejection go unhandled.
  if (request.mode === 'navigate' || FRESH_PAGES.includes(url.pathname)) {
    event.respondWith(
      fetch(request)
        .then((response) => {
          if (response.ok) {
  ...
  // Everything else: cache-first with network fallback.
  // BUG FIX: add .catch() so a network failure doesn't produce an
  // unhandled promise rejection that breaks the service worker.
  event.respondWith(
    caches.match(request).then(
      (cached) =>
        cached ||
