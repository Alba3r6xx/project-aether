 ...
 */
// Cache the user's org_id across subscription calls. Both subscribeToAlerts
// and subscribeToReadings need it, and fetching it twice means two network
// round-trips that double the subscription setup time.
let _cachedOrgId = null;
let _cachedOrgIdPromise = null;

// PERF: get the user ID from the local session (getSession) instead of
// making a network round-trip to Supabase's auth API (getUser). The JWT
// in the session is signed by Supabase and contains the user ID.
...
async function getOrgId() {
  ...
  if (_cachedOrgIdPromise) return _cachedOrgIdPromise;

  _cachedOrgIdPromise = (async () => {
    const userId = await getUserId();
    if (!userId) {
      _cachedOrgIdPromise = null;
      return null;
    }
  ...
  return _cachedOrgIdPromise;
}

// Allow callers to bust the cache on sign-out / org switch.
...
export async function fetchNotificationPreferences() {
  ...
  const userId = await getUserId();
  if (!userId) return { email: true, push: true, weekly_report: false };
...
export async function createAlertRule(rule) {
  ...
  const userId = await getUserId();
  if (!userId) throw new Error('You must be signed in to create alert rules.');
