...
// Lazy-load the mqtt module only when a real POST needs to publish.
// Importing it at the top level can cause WORKER_ERROR on cold starts if
// esm.sh is slow or the module graph fails to resolve — which would crash
// the function before the CORS preflight handler even runs.
let _mqtt: typeof import("https://esm.sh/mqtt@5.10.1") | null = null;
async function getMqtt() {
  if (!_mqtt) {
    _mqtt = await import("https://esm.sh/mqtt@5.10.1");
  }
  return _mqtt;
}
...
// Bound to the request inside the handler so CORS headers can be attached.
...
Deno.serve(async (req: Request) => {
  ...
  // Answer the browser's CORS preflight before the method check — otherwise
  // the OPTIONS request is rejected as 405 with no CORS headers and the real
  // POST is never sent.
  ...
  return jsonResponse(req, { ok: true, topic, requestId });
});
